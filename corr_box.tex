...
% 
% 	\node[strange] at (0,0.5) (k_ti)[label=above:$x_1$,label=left:${1}$] 		 			{};
% 	\node[light] (pi_ti) 			[below of=k_ti,label=below:$x_2$,label=left:${1}$] {}
% 	edge [itof] node[auto,swap]{$u$}(k_ti);
% 	\node[strange] at (2,0.5) (k_tf)[label=above:$y_2$,label=right:${1}$] 		 			{}
% 	
% 	edge [ftoi] node[auto,swap]{$s$}(k_ti)
% 	edge [itof] node[auto,swap]{$u$}(pi_tf);
% 	\node[light] (pi_tf) 			[below of=k_tf,label=below:$y_1$,label=right:${1}$] {}
